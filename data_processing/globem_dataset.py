...
table into windowed multivariate tensors for the same pipeline as HRD. The public entry
point mirrors ``hrd_dataset.prepare_hrd_dataset`` (same return dict), and
``scripts/build_cache.py globem`` writes it to the window cache:
...
Design decisions (see the project discussion):
  ...
  * Missingness: the GLOBEM features are sparse (sleep ~33% present) and CoST's FFT layer
    ...
    (models.encoder.CoSTEncoder) -- a per-channel mask would not function. So each participant's
...
def pid_majority_label(labels):
    """One participant-level label from per-window labels: the majority, ties -> 0."""
...
# Preserve all 14 numeric sensor features in the supplied export. Sensor removal belongs
# to an explicitly recorded ablation; proximity is not assumed irrelevant to rhythmicity.
FEATURE_COLS = [
    ...
    "f_blue:phone_bluetooth_rapids_countscans",
...
#
# NOTE the ranges are NOT hrd_config.WINDOWING.clock_field_ranges: this grid is 4
...
def _to_binary_label(v) -> int:
    ...
    if value in ("false", "0", "0.0"):
        return 0
    raise ValueError(f"unknown binary label {v!r}; do not infer a negative label")
...
def _globem_time_features(dates: pd.DatetimeIndex, start: int, n_steps: int) -> np.ndarray:
    ...
    hrd_windows._cost_time_features but on the 4-segment/day grid: the segment
...
def load_globem(csv_path: str, label_col: str = "LABEL_ENDPOINT",
    ...
    if df.duplicated(["pid", "date", "seg_order"]).any():
        raise ValueError("duplicate GLOBEM participant/date/segment rows")
    if (df.groupby("pid")[label_col].nunique(dropna=True) > 1).any():
        raise ValueError("conflicting GLOBEM endpoint labels within an identifier")
    ...
    print(f"[globem] {len(df):,} rows | {df['pid'].nunique()} participants | "
...
def _prep_participant(g: pd.DataFrame, z_score: bool
    ...
    observed = np.isfinite(arr)                               # original per-channel mask
...
def _window_participant(filled: np.ndarray, observed: np.ndarray, dates: pd.DatetimeIndex,
    ...
    for s in range(start_step, filled.shape[0] - window_steps + 1, stride_steps):
        e = s + window_steps
        if observed[s:e].any(axis=1).mean() < min_window_coverage:
            continue
        arr = filled[s:e]
        if clock_features:
...
def prepare_globem_dataset(
    ...
    hrd_dataset.prepare_hrd_dataset.
    ...
    if weekly_labels:
        raise ValueError("weekly-survey majority is not an endpoint; use a separately specified longitudinal task")
    ...
    if anchor_weekday is not None and anchor_weekday >= 0 and stride_days % 7 != 0:
        print(f"[globem] WARNING: anchor_weekday is set but stride_days="
    ...
    t0 = time.time()
    X_list, y_list, pid_list, wid_list, observed_list, physical_list = [], [], [], [], [], []
    for pid, g in df.groupby("pid", sort=True):
        filled, observed, dates = _prep_participant(g, z_score)
        physical = _prep_participant(g, False)[0] if z_score else filled
        ...
        endpoint_label = label_by_pid.get(pid, -1)
        for arr, win_start in _window_participant(filled, observed, dates, window_steps,
            ...
            start = int((win_start - dates[0]).days) * SEG_PER_DAY
            physical_list.append(physical[start:start + window_steps])
            if keep_observed:
                observed_list.append(observed[start:start + window_steps])
    ...
    print(f"[globem] window start weekday: {_dist}"
    ...
    if clock_features:
        ...
        # the pooled windows including held-out test participants. See _CAL_RANGES above.
    ...
    if weekly_labels:
        ...
        print(f"[globem] built {len(X_list):,} windows of shape "
    ...
    return {
        "X": X,
        "raw_X": np.stack(physical_list).astype(np.float32),
        **({"observed": np.stack(observed_list)} if keep_observed else {}),
        "y": y,
        "pids": pids_arr,
        "pid_ds": pid_ds,
